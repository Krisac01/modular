/*
  # Database Schema Homologation

  1. New Tables
    - Adds missing tables to match application code
    - Ensures consistent naming and structure
    - Adds proper relationships between tables
  
  2. Updates
    - Updates existing tables to match application data models
...
INSERT INTO user_profiles (id, email, name, role, department, status, permissions) 
VALUES (
  gen_random_uuid(), 
  'usuario@ejemplo.com', 
  'Roberto Sánchez', 
...
UPDATE SET name = 'Roberto Sánchez', department = 'Campo';
...
ON CONFLICT (email) DO 
UPDATE SET name = 'Administrador Sistema', department = 'Administración';